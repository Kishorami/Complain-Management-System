...
function doLogin()
{
	...
	if ($userName == '') {
		...
		//check if user is actually employee of the office or not
		if($uType == 'customer')
		{
			...
			if (dbNumRows($result) == 1) {
				$row = dbFetchAssoc($result); //assossiated result gulo ber kore nicchi.
		...
		elseif($uType == 'employee')  // checking if the user is an engineer
		{
...
function doLogout()
{
	...
	if (isset($_SESSION['user_name'])) {
		...
		session_destroy('user_name');
	}
	if (isset($_SESSION['user_type'])) {
		unset($_SESSION['user_type']);
		session_destroy('user_type');
	}
